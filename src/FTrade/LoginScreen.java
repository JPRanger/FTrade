	...
	private void initialize() {
		...
		btnLogin.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){ 
				...
				try{
					loginCheck = new Scanner(new File("accounts/accounts.csv"));
				}
				...
				while(loginCheck.hasNext()){
					...
					if(checkUsername.equals(usernameField.getText() + "," )){//added a comma so username can be found
						System.out.println("Username check success");
						loginInfo[0] = checkUsername;
						String st = loginCheck.next();
						String[] tokens = st.split(",");//getting the password without comma
						StringBuilder sb = new StringBuilder(); //builds new string
						for(int i =0; i <tokens.length;i++)//checks for comma 
						{
							sb.append(tokens[i]);
						}
						String passBuilder = new String (sb); //stores string without comma
						
						char[] passwordConvert =passBuilder.toCharArray();
